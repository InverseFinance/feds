...
contract BaseFedCCTP {
    ...
    IL1ERC20Bridge public constant baseBridge =
        IL1ERC20Bridge(0x3154Cf16ccdb4C6d922629664174b904d80F2C35);
    ...
    function expansionAndSwap(
        ...
        if (useCCTP) {
            ...
            CCTP.depositForBurn(
                usdcAmount,
                BASE_CCTP_DOMAIN,
                bytes32(uint256(uint160(aeroFarmer))),
                address(USDC)
            );
        } else {
     ...
     * @param newAeroFarmer L2 address to be set as aeroFarmer
     */
    function changeAeroFarmer(address newAeroFarmer) external onlyGov {
        aeroFarmer = newAeroFarmer;
    }
}
